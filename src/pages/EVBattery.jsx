...
function ChargeCurve({ points, color, capacity }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '0.68rem', color: 'var(--muted)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Charge rate curve</div>
...
function ConsumptionCurve({ points, color }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '0.68rem', color: 'var(--muted)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Consumption (Wh/km)</div>
      <svg width={W} height={H} style={{ display: 'block', overflow: 'visible' }}>
        <path d={d} stroke={color} strokeWidth="2" fill="none" strokeLinejoin="round" />
        {points.map((p, i) => (
          <circle key={i} cx={toX(p[0])} cy={toY(p[1])} r="3.5" fill={color} />
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {classes.map(c => (
            <button key={c} onClick={() => setCls(c)} style={{
              padding: '8px 18px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10, marginBottom: 20 }}>
          {[
            ...
            <div key={label} style={{ padding: '12px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)' }}>
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--muted)', marginBottom: 2 }}>{label}</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 700, color: p.color }}>{val}</div>
            </div>
          ))}
        </div>
