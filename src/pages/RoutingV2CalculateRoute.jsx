...
const PARAMS_ROUTE = [
  ...
  { name: 'callback', type: 'string', desc: 'JSONP callback function name. Only used when contentType=jsonp.' },
];

const PARAMS_HEADERS = [
  { name: 'TomTom-Api-Key', required: true, type: 'string (header)', desc: 'Your TomTom API key. Passed as a request header rather than a query parameter in v2.' },
  { name: 'Tracking-ID', type: 'string (header)', desc: 'Optional request identifier. Echoed back in the Tracking-ID response header. If both the header and the trackingId query parameter are provided, the query parameter takes precedence.' },
...
/* ─── Code examples ──────────────────────────────────────────────────────── */
const CODE_GET = `# Routing API v2 — Calculate Route (GET)
# Endpoint: /maps/orbis/routing/calculateRoute/{locations}/{contentType}
  ...
  const sections = [
    {
      ...
      method: 'GET',
      note: 'Calculates a route between two or more waypoints. The API key is passed as the TomTom-Api-Key request header — not as a query parameter. Both GET and POST are supported; use POST when you need avoid areas, per-leg overrides, or route reconstruction.',
      params: PARAMS_ROUTE,
      code: CODE_GET,
      lang: 'bash',
    },
    {
      id: 'routing-v2-headers',
    ...
    {
      id: 'routing-v2-combustion',
      heading: 'Combustion consumption model',
      method: 'GET',
      note: 'Requires vehicleEngineType=combustion. Enables fuelConsumptionInLiters in the response summary.',
      params: PARAMS_COMBUSTION,
      code: CODE_COMBUSTION,
      lang: 'bash',
    },
    {
      id: 'routing-v2-electric',
      heading: 'Electric consumption model',
      method: 'GET',
      note: 'Requires vehicleEngineType=electric. Enables batteryConsumptionInkWh in the response. Efficiency parameters and altitude-based parameters are mutually exclusive.',
      params: PARAMS_ELECTRIC,
      code: CODE_ELECTRIC,
      lang: 'bash',
    },
    {
      id: 'routing-v2-post-body',
      heading: 'POST body',
      method: 'POST',
      note: 'POST-only fields. Use a JSON body when you need avoid areas, vignette control, per-leg overrides, or route reconstruction via supporting points.',
      params: PARAMS_POST_BODY,
      code: CODE_POST,
      lang: 'bash',
    },
    {
      id: 'routing-v2-response',
      heading: 'Response & error codes',
      params: [],
      note: 'The response structure mirrors Routing API v1. Route geometry is in legs[].points, traffic and section data in sections[].',
      extra: (
