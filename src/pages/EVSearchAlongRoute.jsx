...
/* ─── Code examples ──────────────────────────────────────────────────────────── */
const CODE = `# EV Search Along Route (v1) — CCS chargers ≥50 kW within 1 km of route corridor
curl --location \\
  "https://api.tomtom.com/search/2/alongRoute?key=YOUR_API_KEY\\
...
export function EVSearchAlongRouteV1Content({ onNavigate }) {
  return (
    <ApiRefTwoCol
      description="Find EV charging stations within a corridor along a pre-calculated route. Ideal for long-distance trips where the vehicle needs one or more charging stops. Optionally enriches results with detour distance and time for each station found."
      sections={[
        {
          id: 'request',
          heading: 'Request',
          method: 'POST',
          note: 'https://api.tomtom.com/search/2/alongRoute — TomTom Maps v1. POST a JSON body containing the route geometry (GeoJSON LineString) and optional search corridor width. No special headers required.',
          params: QUERY_PARAMS,
          code: CODE,
          lang: 'bash',
        },
